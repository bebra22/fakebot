...
def start(message):
	bot.send_message(message.chat.id, '''👋 Привет! 👋
		Это бот, который, может показать информацию по юзернейму в телеграмме 
	Для поиска информации, введите команду /getinfo''') 

@bot.message_handler(commands=['getinfo'])
def start(message):
	msg = bot.send_message(message.chat.id, 'Введите юзернейм') 
...
def proc2(message):
	try:
		...
		if not num.isdigit():
			msg = bot.reply_to(message, 'Кажется, вы не ввели действительный юзернейм, повторите попытку, написав /getinfo!')#⏳
			return

		bot.send_message(m_id, f'Запрос на пользователя отправлен!')
		time.sleep(2)
		...
		bot.send_message(m_id, '''Похоже у вас не осталось бесплатных запросов на день!
			Чтобы получить дополнительные вопросы оставте положительный отзыв!''', reply_markup=keyboard)
# Отловка ошибок
...
def callback_inline(call):
	if call.data == "find":
		...
		bot.send_message(call.message.chat.id, text='Для просмотра скрытой информации нажмите Подтвердить', reply_markup=keyboard1)
